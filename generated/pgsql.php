 ...
 * @param  $length An optional maximum number of bytes to write.  Must be greater than zero
 ...
 * 
 */
function pg_lo_write(\PgSql\Lob $lob, string $data,  $length = null): int
{
    error_clear_last();
...
{
    error_clear_last();
    $result = \pg_set_chunked_rows_size($connection, $size);
    if ($result === false) {
        throw PgsqlException::createFromPhpError();
    }
}
...
{
    error_clear_last();
    $result = \pg_trace($filename, $mode, $connection, $trace_mode);
    if ($result === false) {
        throw PgsqlException::createFromPhpError();
    }
}
