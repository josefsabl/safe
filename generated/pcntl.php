...
{
    error_clear_last();
    $result = \pcntl_getcpuaffinity($pid);
    if ($result === false) {
 ...
 * 
 * @param  $process_id If NULL, the process id of the current process is used.
 ...
 * 
 */
function pcntl_getpriority( $process_id = null, int $mode = PRIO_PROCESS): int
{
    error_clear_last();
...
function pcntl_setcpuaffinity( $pid = null, array $hmask = null): void
{
    error_clear_last();
    if ($hmask !== null) {
        $result = \pcntl_setcpuaffinity($pid, $hmask);
    }else {
        $result = \pcntl_setcpuaffinity($pid);
    }
    if ($result === false) {
        throw PcntlException::createFromPhpError();
    }
 ...
 * 
 */
function pcntl_setpriority(int $priority,  $process_id = null, int $mode = PRIO_PROCESS): void
{
    error_clear_last();
 ...
 * @param array $old_signals The old_signals parameter is set to an array
 ...
 * @param array $info The info is set to an array containing
 ...
 * 
 * @param array $signals Array of signals to wait for.
 * @param array $info The info parameter is set to an array containing
