 ...
 * @param array $image_info This optional parameter allows you to extract some extended
 ...
 * 
 */
function imagefilter(\GdImage $image, int $filter,  $args): void
{
    error_clear_last();
 ...
 * 
 */
function imageresolution( $image,  $resolution_x = null,  $resolution_y = null)
{
    error_clear_last();
...
{
    error_clear_last();
    $result = \imagerotate($image, $angle, $background_color);
    if ($result === false) {
        throw ImageException::createFromPhpError();
    }
    return $result;
 ...
 * such as imagecreatetruecolor.
 * @param  $file The path or an open stream resource (which is automatically closed after this function returns) to save the file to. If not set or NULL, the raw image stream will be output directly.
 * @param  $foreground_color You can set the foreground color with this parameter by setting an
 * identifier obtained from imagecolorallocate.
 * The default foreground color is black.
 * @throws ImageException
 * 
 */
function imagewbmp(\GdImage $image,  $file = null,  $foreground_color = null): void
{
    error_clear_last();
 ...
 * 
 */
function imagexbm(\GdImage $image, ?string $filename,  $foreground_color = null): void
{
    error_clear_last();
